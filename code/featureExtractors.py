...
class GhostExtractor(FeatureExtractor):
    ...
    - whether a ghost collision is imminent
    - whether a ghost is one step away
    - manhattan distance of all ghosts
    """
    ...
    def getFeatures(self, state, action):
        ...
        ghostx, ghosty = zip(*ghosts)
        mingx = min(ghostx)
        maxgx = max(ghostx)
        mingy = min(ghosty)
        maxgy = max(ghosty)

        features["x-sandwich"] == -10 if x <= maxgx and x >= mingx else 10
        features["y-sandwich"] == -10 if x <= maxgy and y >= mingy else 10

        # count the number of ghosts 1-step away
        one_step_away = sum(
            (next_x, next_y) in Actions.getLegalNeighbors(g, walls) for g in ghosts)
        features["one-away"] = one_step_away
        # if there is no danger of ghosts then add the food feature
        if one_step_away == 0 and food[next_x][next_y]:
            features["eats-food"] = 1.0
