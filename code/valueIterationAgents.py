...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for i in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                for action in legalActions:
                    ...
                    bestValue = max(qValue, bestValue)
    ...
    def computeActionFromValues(self, state):
        ...
        """
        legalActions = self.mdp.getPossibleActions(state)
        bestValue = (-math.inf, -1, None)
        for (i, action) in enumerate(legalActions):
            qValue = self.getQValue(state, action)
            bestValue = max((qValue, i, action), bestValue)
        return bestValue[2]
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        state_iterator = enumerate(itertools.cycle(states))
        for (i, state) in state_iterator:
